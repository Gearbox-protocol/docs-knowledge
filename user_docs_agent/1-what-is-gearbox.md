...
On the Borrow and Credit Account pages the leverage slider currently tops out near **10×**, so real maximum leverage is roughly ten-fold but varies slightly by asset and credit manager.

SCREENSHOT: /images/dashboard/main-screen.png — Gearbox dashboard showing the “In Pools” and “On Credit Accounts” summary cards and the “Your Credit Accounts” section
...
The UI emphasises that there are **no funding rates**—for example the Dashboard card titled “Margin Trading With No Funding Rates” highlights that borrow positions use spot liquidity and do not accrue periodic funding fees.
...
On the Dashboard a banner invites users to “Upgrade to Permissionless for a smoother UX and earn rewards”, confirming that Gearbox currently runs in a **Permissionless** configuration with institutional-grade lending markets curated at the smart-contract level.
...
In the UI the **Health factor** is shown in the right-hand summary panel when opening or managing a Credit Account. For example, on the “crvUSD Credit Account” form the health factor, liquidation price and time to liquidation appear together above the borrow rate.
...
When opening a Credit Account the manual selection modal lists each **Credit Manager** with minimum and maximum debt ranges. For example, *crvUSD Tier 1* allows 0.00 k–1.00 M crvUSD of debt, *DAI Tier 1* lists 20.00 k–100 M DAI, *Trade DOLA Tier 1* shows 20.00 k–400.00 k DOLA, and the *hemiBTC* manager accepts roughly 0.10–5.00 hemiBTC. These limits are subject to change and vary across assets.
...
On the Credit Account form the variable **Borrow rate** is displayed as a single percentage. In the Permissionless version there are no gauge fees; instead the borrow rate may include a risk premium set by the curator.
...
Borrow interest rates are shown in the right-hand summary panel of the Credit Account form under **Borrow rate**. Deposit forms display the **Supply APY** for lenders but do not include any Gearbox fees since lenders earn interest and pay none.
...
TODO: link to official Risks & Security page (not surfaced in current UI, requires docs maintainers).
TODO: link to FAQ page in the current docs structure (not surfaced in current UI, requires docs maintainers).
