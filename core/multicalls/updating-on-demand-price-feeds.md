...
// Price updates must be first in the multicall array
const priceUpdateCall = {
  ...
  callData: encodeFunctionData({
    ...
    args: [
      tokenAddress,    // token with on-demand price feed
      false,           // false = main feed, true = reserve feed
      priceData        // authenticated payload from Pyth/Redstone
    ]
  })
};

// For withdrawals, you may also need to update the reserve feed
const reservePriceCall = {
  target: creditFacade,
  callData: encodeFunctionData({
    abi: iCreditFacadeV3MulticallAbi,
    functionName: 'onDemandPriceUpdate',
    args: [tokenAddress, true, reservePriceData]
  })
};

// Price updates must precede all other calls
const calls = [priceUpdateCall, reservePriceCall, ...otherCalls];
```

> For oracle architecture and on-demand feed mechanics, see [Smart Oracles](../../../new-docs-about/economics-and-risk/smart-oracles/).

### Updating on-demand price feeds

When a token has only an on-demand price feed and is enabled as collateral on the Credit Account, any multicall on this account requires updating the price (it's technically possible that the last price is still usable, as it remains "fresh" for a few minutes after the update - however, it is best to push the price regardless just to be sure). If all required prices are not updated at the beginning, the collateral check will fail. Moreover, multicalls with withdrawals may require updating reserve price feeds as well.
