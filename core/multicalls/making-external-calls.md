...
// Get adapter address from Credit Manager
const yearnUsdcAdapter = await creditManager.read.contractToAdapter([yvUSDC]);
...
await creditFacade.write.multicall([creditAccountAddress, calls]);
```
...
The specifications for each adapter type and usage instructions can be found in the corresponding section.
...
### Common Adapter Patterns in TypeScript

```typescript
import { encodeFunctionData } from 'viem';

// Uniswap V3 exact input swap
const uniswapAdapter = await creditManager.read.contractToAdapter([uniswapRouter]);
const swapCall: MultiCall = {
  ...
  callData: encodeFunctionData({
    ...
    args: [{
      ...
      amountIn: amount,
      amountOutMinimum: 0n, // Use storeExpectedBalances for slippage
      sqrtPriceLimitX96: 0n,
    }],
  }),
};

// Curve swap using "diff" pattern (swap entire balance minus leftover)
const curveAdapter = await creditManager.read.contractToAdapter([curvePool]);
const curveDiffCall: MultiCall = {
  target: curveAdapter,
  callData: encodeFunctionData({
    abi: curveAdapterAbi,
    functionName: 'exchange_diff',
    args: [
      0,  // i (from token index)
      1,  // j (to token index)
      1n, // leftoverAmount (leave 1 wei)
    ],
  }),
};

// Convex stake
const convexAdapter = await creditManager.read.contractToAdapter([convexBooster]);
const convexStakeCall: MultiCall = {
  target: convexAdapter,
  callData: encodeFunctionData({
    abi: convexAdapterAbi,
    functionName: 'depositDiff',
    args: [true], // stake = true
  }),
};
```
