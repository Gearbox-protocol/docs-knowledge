...
// Define expected minimum output after swap
const balanceDeltas = [
  { token: outputToken, amount: expectedMinOutput }
];
...
// 3. Compare balances (reverts if slippage exceeded)
const compareCall = {
  target: creditFacade,
  callData: encodeFunctionData({
    abi: iCreditFacadeV3MulticallAbi,
...
// Execute multicall with slippage protection
const calls = [storeCall, swapCall, compareCall];
```
