...
await token.write.approve([creditManagerAddress, amount]);
...
// Sign permit message (EIP-2612)
const deadline = BigInt(Math.floor(Date.now() / 1000) + 3600);
const signature = await walletClient.signTypedData({
  // ... permit typed data for token
});
...
// No approve needed - permit handles authorization
await creditFacade.write.multicall([creditAccountAddress, calls]);
```
