...
import { iCreditFacadeV3MulticallAbi, iCreditManagerV3Abi } from '@gearbox-protocol/sdk';

// Get token masks for collateral hints
...
// Order by expected contribution to TWV (highest value first)
const collateralHints = [usdcMask, wethMask];

// Set minimum health factor (in basis points, 10000 = 100%)
const minHealthFactor = 10500n; // 105% minimum health factor
...
const setParamsCall = {
  ...
  callData: encodeFunctionData({
    ...
    args: [collateralHints, minHealthFactor]
  })
};

// Place setFullCheckParams before other calls in the multicall
const calls = [setParamsCall, ...operationCalls];
```
