...
const withdrawAllCall = {
  target: creditFacade,
  callData: encodeFunctionData({
    abi: iCreditFacadeV3MulticallAbi,
    functionName: 'withdrawCollateral',
    args: [tokenAddress, maxUint256, recipientAddress]
  })
};
```
...
where $p\_i^M$ is the main price feed value and $p\_i^R$ is the reserve price feed value. Depending on how the reserve price feed is set up, it's value may be always the same as the main price feed (in this case we consider the asset "trusted"), lower than the main price feed, or even 0. As a result, some asset compositions on a Credit Account may make withdrawals impossible due to the collateral check with "safe prices" failing.
